...
public class LogServiceTest {
    ...
    public void POST_요청시_LogSaveDto가_생성되어_이벤트가_발행된다() throws Throwable {
        ...
        when(joinPoint.getArgs()).thenReturn(new Object[]{});//비즈니스 로직으로 전달
    ...
    @Test
    public void LOGOUT_요청시_LogSaveDto가_생성되어_이벤트가_발행된다() throws Throwable {
        // given

        // SecurityContextHolder에 userId(Long)만 Principal로 설정
        CustomUserDetails userDetails = mock(CustomUserDetails.class);
        when(userDetails.getId()).thenReturn(1L);
        Authentication auth = new UsernamePasswordAuthenticationToken(userDetails, null, null);
        SecurityContextHolder.getContext().setAuthentication(auth);
        ...
        assertThat(dto.getMethod()).isEqualTo("POST");
        assertThat(dto.getUri()).isEqualTo("/api/logout");
        assertThat(dto.getContents()).isEqualTo("로그아웃 되었습니다.");
